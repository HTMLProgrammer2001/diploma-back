...
export class InternshipFiller implements FillerInterface {
  ...
  async fill(workbook: Workbook, data: ExportDataInterface): Promise<void> {
    ...
    for(let i = 0; i < data.internshipData.length; i++) {
      ...
      worksheet.getRow(row).getCell(InternshipColumnsEnum.FROM).value = dateToString(internshipData.from);
      worksheet.getRow(row).getCell(InternshipColumnsEnum.TO).value = dateToString(internshipData.to);
